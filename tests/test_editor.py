...
@pytest.mark.parametrize(
    "raw, expected",
    [
        ("Ну, короче, надо в среду.", "Надо в среду."),
        ("Короче, объясни, что такое рекурсия.", "Объясни, что такое рекурсия."),
        ("короче, скидка, ну, десять процентов.", "Скидка, ну, десять процентов."),
        ("Готово. Короче, отправляй.", "Готово. Отправляй."),
        ("В общем, всё готово!", "Всё готово!"),
        # паразит в середине фразы убирает уже ИИ
        ("Нужно, короче, двадцать кубов.", "Нужно, короче, двадцать кубов."),
        # без запятой это обычные слова
        ("Типа USB-C нужен.", "Типа USB-C нужен."),
        ("Нужен разъём типа USB-C.", "Нужен разъём типа USB-C."),
        ("Вот, что я думаю.", "Вот, что я думаю."),
        ("Нужно проверить.", "Нужно проверить."),
    ],
)
def test_drop_leading_fillers(raw, expected):
    assert editor._drop_leading_fillers(raw) == expected


# --- защита уровня 2: он исправляет ошибки, но не переписывает текст ---
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # исправлено окончание
        ("Документы, который я отправил вчера, лежат в общей папке.",
         "Документы, которые я отправил вчера, лежат в общей папке."),
        ("Ответ пришлём в течении недели согласно графика.", "Ответ пришлём в течение недели согласно графику."),
        # исправлено слово, которое распознавание записало с ошибкой, и расставлены знаки
        ("Нужно 25 кубов бетона привести 5 марта я думаю что подрядчик успеет.",
         "Нужно 25 кубов бетона привезти 5 марта. Я думаю, что подрядчик успеет."),
        # убран лишний предлог, исправлено окончание
        ("Мы обсудили с подрядчиком о сроках.", "Мы обсудили с подрядчиком сроки."),
        # е и ё - одна буква
        ("Всё готово.", "Все готово."),
    ],
)
def test_only_small_changes_allows(before, after):
    assert editor.only_small_changes(before, after)


@pytest.mark.parametrize(
    "before, after",
    [
        # слово заменено другим
        ("Документы готовы, затем позвоню.", "Документы готовы. Тогда позвоню."),
        ("Надо созвониться в четверг.", "Нужно созвониться в четверг."),
        # слово выброшено
        ("В разделе АР-2 поменяли планировку, поэтому смету пересчитать.",
         "В разделе АР-2 поменяли планировку. Смету пересчитать."),
        # "не" выброшено - смысл стал обратным
        ("Кран ещё не поставили.", "Кран ещё поставили."),
        # слова добавлены
        ("Указать номер договора и сумму долга.", "Указать номер договора. Нужно указать сумму долга."),
        ("Позвони Ивану.", "Позвони Ивану, пожалуйста."),
        # число изменилось
        ("Нужно 25 кубов бетона.", "Нужно 20 кубов бетона."),
        ("Вышло 120 000 ₽.", "Вышло 120000 ₽."),
        # исправлений слишком много для такой короткой фразы
        ("Он сделал это быстро и хорошо.", "Она сделала эту быстро и хорошо."),
        # модель ответила вместо редактуры
        ("Напиши функцию, которая сортирует список.", "def sort(items): return sorted(items)"),
    ],
)
def test_only_small_changes_rejects(before, after):
    assert not editor.only_small_changes(before, after)


def test_prompt_files_exist():
    for name in (editor.LEVEL1_PROMPT, editor.LEVEL2_PROMPT):
        assert (editor.PROMPTS_DIR / name).exists()
